...
const urlToPath = (baseDir, url) => {
  ...
  const filePath = path.join(baseDir, `${pathname}.html`);
  return filePath;
};

const urlToLocalHref = (url) => {
  const parsed = new URL(url);
  let pathname = parsed.pathname.replace(/\/$/, ""); // remove trailing slash
  if (pathname === "") pathname = "/index";
  return `/scraped_website${pathname}.html`;
};
...
await page.evaluate((newSources) => {
  ...
  imgs.forEach((img, i) => {
    ...
    // Replace all a[href] with new paths
    ...
    // Remove script tags by commenting them out
    ...
    const filePath = urlToPath(baseDir, url);
