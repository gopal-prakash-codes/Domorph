...
export const replaceElement = tool(
  async ({ ModifiedElement, fileName, domain, xpath }) => {
    console.log(
      `Replacing ModifiedElement ${ModifiedElement} in ${fileName} for ${domain} at XPath: ${xpath}`
    );
  ...
  },
  {
    name: "replaceElement",
    description:
      "Replaces an element in a local HTML file identified by XPath with the new HTML content.",
    schema: z.object({
      ModifiedElement: z
        .string()
        .describe("The new HTML string to replace the target element with"),
      fileName: z.string().describe("HTML file name (e.g., index.html)"),
      domain: z.string().describe("Name of the domain"),
      xpath: z
        .string()
        .describe("XPath expression identifying the element to replace"),
    }),
  }
);
